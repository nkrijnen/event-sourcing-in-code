...
internal class LegoSetAggregateTraditionalTest {
    ...
    fun `should always start with added-to-catalog event if there are events for this set`() {
        assertDoesNotThrow {
            LegoSetAggregateTraditional(listOf())
        }
    ...
    inner class BookingLegoSets {
        ...
        fun `should book set when available`() {
            ...
            Assertions.assertThat(event.atTime).isCloseTo(Instant.now(), Assertions.byLessThan(1, ChronoUnit.SECONDS))
        }

        @Test
        fun `should fail to book set when not in catalog`() {
            val legoSet = LegoSetAggregateTraditional(listOf())
...
private fun List<LegoSetEvent>.toLegoSet() = LegoSetAggregateTraditional(this)